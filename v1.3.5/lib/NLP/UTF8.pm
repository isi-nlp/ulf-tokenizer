...
sub character_type {
   ...
   if ($c =~ /^[\x00-\x7F]/) {
      ...
      return "Arabic currency" if $c =~ /\xDB\xB8/; # Afghani sign
      ...
      return "Oriya digit" if $c =~ /\xE0\xAD[\xB2-\xB7]/; # fraction
      return "Oriya letter" if $c =~ /\xE0[\xAC-\xAD]/;
      return "Tamil digit" if $c =~ /\xE0\xAF[\xA6-\xAF]/;
      return "Tamil digit" if $c =~ /\xE0\xAF[\xB0-\xB2]/; # number (10, 100, 1000)
      return "Tamil letter" if $c =~ /\xE0[\xAE-\xAF]/;
      return "Telegu digit" if $c =~ /\xE0\xB1[\xA6-\xAF]/;
      return "Telegu digit" if $c =~ /\xE0\xB1[\xB8-\xBE]/; # fraction digit
      ...
      return "Korean Hangul character" if $c =~ /\xE1[\x84-\x87]/;
      return "Ethiopic punctuation" if $c =~ /\xE1\x8D[\xA0-\xA8]/;
      return "Ethiopic digit" if $c =~ /\xE1\x8D[\xA9-\xBC]/;
      ...
      return "non-ASCII whitespace"  if $c =~ /\xE2\x80[\x80-\x8A]/;
      ...
      return "directional mark"      if $c =~ /\xE2\x80[\x8E-\x8F]/;
      ...
      return "Coptic digit" if $c =~ /\xE2\xB3\xBD/; # fraction 1/2
      ...
      return "non-ASCII punctuation" if $c =~ /\xE3\x80[\x80-\xA0]/;
      return "Japanese kana character" if $c =~ /\xE3[\x81-\x83]/;
      ...
      return "Korean Hangul character" if $c =~ /\xEA[\xB0-\xBF]/;
      return "Korean Hangul character" if $c =~ /[\xEB-\xEC]/;
      return "Korean Hangul character" if $c =~ /\xED[\x80-\x9E]/;
      ...
      return "Phoenician digit" if $c =~ /\xF0\x90\xA4[\x96-\x9B]/; # number
      return "Phoenician punctuation" if $c =~ /\xF0\x90\xA4\x9F/; # word separator
      return "Old Hungarian digit" if $c =~ /\xF0\x90\xB3[\xBA-\xBF]/; # number
      ...
      return "pictograph" if $c =~ /\xF0\x9F[\x8C-\xA7]/;
