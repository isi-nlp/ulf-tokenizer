...
sub guard_urls_x045 {
   ...
   $result =~ s/((?:\b(?:fig))(?:_DONTBREAK_)?\.?|\b(?:figures?|tables?|chromosomes?)|<xref\b[^<>]*\b(?:fig)\b[^<>]*>)\s*(\d+[a-z](?:(?:-|\xE2\x80\x93|\xE2\x80\x94)[a-z]{1,2})?)\b/$1 \x04$2\x05/gi;
...
$georgian = "(?:\xE1\x82[\xA0-\xBF]|\xE1\x83[\x80-\xBF]|\xE1\xB2[\x90-\xBF]|\xE2\xB4[\x80-\xAF])";

$letter_digit_or_punct = "(?:[\x21-\x7F]|$alpha|$cyrillic|$georgian)";
sub restore_urls_x045_guarded_string {
   ...
   while (($pre, $url, $post) = ($s =~ /^(.*?)\x04([^\x04\x05]*?)\x05(.*)$/)) {
      ...
      $url =~ s/ //g;
      $url =~ s/\x02/ /g;
      # avoid symbols sticking to letters, digits, punct
      $sep1 = (($pre =~ /$letter_digit_or_punct$/) || ($url =~ /^$letter_digit_or_punct/)) ? " " : "";
      $sep2 = (($url =~ /$letter_digit_or_punct$/) || ($post =~ /^$letter_digit_or_punct/)) ? " " : "";
      $s = "$pre$sep1$url$sep2$post";
   }
   if ($s =~ /[\x04\x05]/) {
      print STDERR "Removing unexpectedly unremoved x04/x05 marks from $s\n";
      $s =~ s/[\x04\x05]//g;
   }
   $s =~ s/(\S)\@(-|\xE2\x80\x93|\xE2\x80\x94)\@(\S)/$1$2$3/g;
   return $s;
}
...
sub tokenize {
   ...
   my $s = $utf8->repair_misconverted_windows_to_utf8_strings($s);
   print "Point A2: $s\n" if $local_verbose;
   ...
   $s =~ s/(?:\xC2\xA0|\xE2\x80[\x80-\x8A]|\xE2\x80\xAF|\xE2\x81\x9F|\xE3\x80\x80)/ /g; # non-ASCII spaces
   ...
   $s =~ s/(\d)([a-z][A-z])/$1 $2/g;
   $s =~ s/(\w|`|'|%|[a-zA-Z]\.|[a-zA-Z]_DONTBREAK_\.)(-|\xE2\x80[\x93\x94])(\w|`|')/$1 \@$2\@ $3/g;
   $s =~ s/(\w|`|'|%|[a-zA-Z]\.|[a-zA-Z]_DONTBREAK_\.)(-|\xE2\x80[\x93\x94])(\w|`|')/$1 \@$2\@ $3/g;
   $s =~ s/(\w)- /$1 \@- /g;
   $s =~ s/ -(\w)/ -\@ $1/g;
   $s =~ s/(\d):(\d)/$1 \@:\@ $2/g;
   ...
   if ($s =~ /$georgian/) { # added Apr. 23, 2021
      for (my $i=0; $i <= 1; $i++) {
         $s =~ s/($georgian)([,;:!?.\(\)\[\]\/]|\xD8\x8C|\xD8\x9B|\xD8\x9F|\xD9\xAA|\xC2\xAB|\xC2\xBB|\xE2[\x80-\x9F][\x80-\xBF])/$1 $2/gi; # punctuation includes Arabic ,;?%
         $s =~ s/([,;:!?.\(\)\[\]\/]|\xD8\x8C|\xD8\x9B|\xD8\x9F|\xD9\xAA|\xC2\xAB|\xC2\xBB|\xE2[\x80-\x9F][\x80-\xBF])($georgian)/$1 $2/gi;
      }
      $s =~ s/(\d|[a-zA-Z]|$georgian)([-]|\xE2\x80[\x93\x94])($georgian)/$1 \@$2\@ $3/g;
      $s =~ s/( )([-]|\xE2\x80[\x93\x94])($georgian)/$1$2\@ $3/g;
      $s =~ s/($georgian)([-]|\xE2\x80[\x93\x94])( )/$1 \@$2$3/g;
      $s =~ s/(\d|[a-zA-Z])($georgian)/$1 \@\@ $2/g;
      $s =~ s/([,;:()'*%|]|\xE2\x80\x99)($georgian)/$1 $2/g;
      $s =~ s/([,;:()'*%|])([-]|\xE2\x80[\x93\x94])($georgian)/$1 \@$2\@ $3/g;
      $s =~ s/([ 0-9.,;:?!()'*|][.+\\]+)($georgian)/$1 $2/g;
      $s =~ s/([ 0-9.,;:?!()'*|][.+\\%]+)([-]|\xE2\x80[\x93\x94])($georgian)/$1 \@$2\@ $3/g;
      $s =~ s/($georgian)([,;:<>()%'`*^#|\\])/$1 $2/g;
      $s =~ s/($georgian)([-+_&])/$1 \@$2/g;
      $s =~ s/($georgian)(\.)([-,\/\\])/$1$2 $3/g;
      $s =~ s/($georgian)(_+ )/$1 $2/g;
      $s =~ s/($georgian)(\.{2,} )/$1 $2/g;
      $s =~ s/( [.'′\@]*(?:[+_,<>`&]|\xC2\xB7)+)($georgian)/$1 $2/g; # middle dot
      $s =~ s/(\/-)($georgian)/$1 $2/g;
    # $s =~ s/($cyrillic$lc_cyrillic)(\.)($uc_cyrillic$lc_cyrillic)/$1 $2 $3/g;
   }
