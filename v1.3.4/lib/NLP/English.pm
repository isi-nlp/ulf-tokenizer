...
sub noun_number_form {
   ...
   return $noun if $noun =~ /\b(yuan|renminbi|RMB|rand|won|yen|ringgit)$/;  # no change in plural
...
sub guard_urls_x045 {
   ...
   $s = $result;
   $result = "";
   while (($pre, $hashtag, $post) = ($s =~ /^(.*?[ .,;()\[\]{}'])([#@](?:[a-z]|\xC3[\x80-\x96\x98-\xB6\xB8-\xBF])(?:[_a-z0-9]|\xC3[\x80-\x96\x98-\xB6\xB8-\xBF])*(?:[a-z0-9]|\xC3[\x80-\x96\x98-\xB6\xB8-\xBF]))(.*)$/i)) {
...
sub guard_abbreviations_with_dontbreak {
   ...
   my $orig = $s;
   my $result = "";
   while (($pre,$potential_abbrev,$period,$post) = ($s =~ /^(.*?)((?:[a-z]+\.-?)*(?:[a-z]|\xC3[\x80-\x96\x98-\xB6\xB8-\xBF])+)(\.)(.*)$/i)) {
...
$alpha = "(?:[a-z]|\xCE[\xB1-\xBF])";
$alphanum = "(?:[a-z0-9]|\xCE[\xB1-\xBF])(?:[-_a-z0-9]|\xCE[\xB1-\xBF])*(?:[a-z0-9]|\xCE[\xB1-\xBF])|(?:[a-z0-9]|\xCE[\xB1-\xBF])";
...
sub tokenize {
   ...
   $s =~ s/([a-zA-Z%\/\[\]]|\x05|[a-zA-Z]_DONTBREAK_\.)([,;:!?])\s*(\S)/$1 $2 $3/g;
   # asterisk
   $s =~ s/( [(\[]?)(\*)([a-z0-9])/$1$2\@ $3/gi;
   $s =~ s/([a-z0-9])(\*)([.,;:)\]]* )/$1 \@$2$3/gi;
   print "Point J: $s\n" if $local_verbose;
   ...
   $s =~ s/([a-zA-Z%]|\x05)([,;:.!?])([")]|''|\xE2\x80[\x99\x9D]|)(\s)/$1 $2 $3$4/g;
   $s =~ s/([a-zA-Z%]|\x05)([,;:.!?])([")]|''|\xE2\x80[\x99\x9D]|)\s*$/$1 $2 $3/g;
   $s =~ s/([.,;:]|\x05)('|\xE2\x80[\x99\x9D])/$1 $2/g;
   $s =~ s/('|\xE2\x80[\x99\x9D])([.,;:]|\x04)/$1 $2/g;
   $s =~ s/([(){}\[\]]|\xC2\xB1)/ $1 /g;
   $s =~ s/([a-zA-Z0-9]|\x05)\.\s*$/$1 ./g;
   $s =~ s/([a-zA-Z]|\x05)\.\s+/$1 . /g;
   $s =~ s/([a-zA-Z]|\x05)\.(\x04)/$1 . $2/g;
   $s =~ s/([0-9]),\s+(\S)/$1 , $2/g;
   $s =~ s/([a-zA-Z])(\$)/$1 $2/g;
   ...
   print "Point Q: $s\n" if $local_verbose;
   foreach $_ ((1 .. 2)) {
      $s =~ s/(\s[-a-z0-9()']*)(\.{2,})([-a-z0-9()?!:\/']*\s|[-a-z0-9()'\/]+\.\s)/$1 $2 $3/gi;
   }
   $s =~ s/0s\b/0 s/g;
   $s =~ s/([0-9])(\x04)/$1 $2/g;
   $s =~ s/ +/ /g;
