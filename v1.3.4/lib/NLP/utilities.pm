...
";
  # START EXCLUDE FROM RELEASE UROMAN
  } elsif ($function_name eq "posteditAMR") {
   $add_javascript_function_s .= "
    function posteditAMR(snt_id,username,workset_name,button_tr_id,control) {
       if (control == 'undefined') {
	  control = '';
       }
       if ((s = document.getElementById(button_tr_id)) != null) {
	  s.style.backgroundColor = \"#7799FF\";
	  var logsid = get_logsid();
	  if ((logsid == '') && ((s2 = document.getElementById('logsid')) != null)) {
	     logsid = s2.value;
	  }
	  var newwindow=window.open('','postedit:' . snt_id);
	  if (control.match(/no-new-focus/)) {
	     // window.focus();
	  ...
	  var tmp = newwindow.document;
	  tmp.write('<html>\\n  <head><title>postedit</title></head>\\n');
	  tmp.write('  <body style=\"background-color:#FFFFEE;\" onload=\"document.getElementById(\\'postedit-amr\\').submit();\">\\n');
	  tmp.write('    <form name=\"postedit-amr\" id=\"postedit-amr\" enctype=\"multipart/form-data\" action=\"https://www.isi.edu/cgi-bin/div3/mt/amr-editor/load-amr.cgi\" method=\"post\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"workset-p\"     name=\"workset-p\"     value=\"1\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"logsid\"        name=\"logsid\"        value=\"' + logsid       + '\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"load-username\" name=\"load-username\" value=\"' + username     + '\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"workset-name3\" name=\"workset-name3\" value=\"' + workset_name + '\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"current-workset-snt-id\" name=\"current-workset-snt-id\" value=\"' + snt_id + '\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"amr-editor-load-command\" name=\"amr-editor-load-command\" value=\"Load AMR at ISI for post-editing\">\\n');
	  tmp.write('      <input type=\"hidden\" id=\"load-amr-editor-file\" name=\"load-amr-editor-file\" value=\"/nfs/isd/ulf/public_html/amr/AMR-editor-c.html\">\\n');
	  tmp.write('    </form>\\n');
	  tmp.write('  </body>\\n');
	  tmp.write('</html>\\n');
	  tmp.close();
...
";
  } elsif ($function_name eq "get_logsid") {
   $add_javascript_function_s .= "
    function get_logsid () {
...
";
  } elsif ($function_name eq "set_workset_status") {
   $add_javascript_function_s .= "
    function set_workset_status (username, workset) {
       var logsid = get_logsid();
       if ((logsid == '') && ((s2 = document.getElementById('logsid')) != null)) {
          logsid = s2.value;
       }
       newwindow= window.open('','Set workset status','height=360,width=720,resizable=1,scrollbars=1,toolbar=1,statusbar=1,menubar=1');
       newwindow.focus();
       var tmp = newwindow.document;
       tmp.write('<html>\\n  <head><title>Set workset status</title></head>\\n');
       tmp.write('  <body style=\"background-color:#FFFFEE;\">\\n');
       tmp.write('    <h3>Update AMR Workset Status</h3>\\n');
       tmp.write('    <form name=\"set-status\" id=\"set-status\" enctype=\"multipart/form-data\" action=\"https://www.isi.edu/cgi-bin/div3/mt/amr-editor/set-status.cgi\" method=\"post\">\\n');
       tmp.write('      <table>\\n');
       tmp.write('        <tr><td>Workset:</td><td><input type=\"input\" id=\"workset-id\" name=\"workset-id\" value=\"' + workset +'\" size=\"51\"></td></tr>\\n');
       tmp.write('        <tr><td>Annotator:</td><td><input type=\"input\" id=\"username\" name=\"username\" value=\"'+ username + '\" size=\"51\"></td></tr>\\n');
       tmp.write('        <tr><td><nobr>New status:</nobr></td><td><select id=\"new-status\" name=\"new-status\"><option value=\"inactive and incomplete\">inactive and incomplete</option><option value=\"annotation in progress\">annotation in progress</option><option value=\"annotation complete\" selected>annotation complete &nbsp; (incl. AMR Checker) &nbsp;</option><option value=\"annotation rejected\">annotation rejected</option><option value=\"postediting in progress\">postediting in progress</option><option value=\"postediting complete\">postediting complete</option><option value=\"postediting rejected\">postediting rejected</option><option value=\"iQC in progress\">iQC in progress</option><option value=\"iQC complete\">iQC complete</option><option value=\"iQC rejected\">iQC rejected</option></select></td></tr>\\n');
       tmp.write('        <tr><td>Comment:<br><span style=\"color:#999999;\">(optional)</span></td><td><textarea rows=\"3\" cols=\"60\" id=\"comment\" name=\"comment\" style=\"overflow:auto;\"></textarea></td></tr>\\n');
       tmp.write('        <tr><td colspan=\"2\"><input type=\"checkbox\" id=\"email-notify\" name=\"email-notify\" checked=\"checked\"> Notify supervisor by email.</td></tr>\\n');
       tmp.write('        <tr><td colspan=\"2\"><input type=\"submit\" value=\"update status\"></td></tr>\\n');
       tmp.write('      </table>\\n');
       tmp.write('      <input type=\"hidden\" id=\"logsid\" name=\"logsid\" value=\"' + logsid + '\">\\n');
       tmp.write('    </form>\\n');
       tmp.write('  </body>\\n');
...
";
  } elsif ($function_name eq "open_amr_issue") {
   $add_javascript_function_s .= "
    function open_amr_issue (annotator, snt_id, username, button_tr_id) {
       if ((s = document.getElementById(button_tr_id)) != null) {
	  s.style.backgroundColor = \"#7799FF\";
       }
       var logsid = get_logsid();
       ...
       newwindow= window.open('','Open issue','height=280,width=700,resizable=1,scrollbars=1,toolbar=1,statusbar=1,menubar=1');
       newwindow.focus();
       var tmp = newwindow.document;
       tmp.write('<html>\\n  <head><title>Open issue</title></head>\\n');
       tmp.write('  <body style=\"background-color:#FFFFEE;\">\\n');
       tmp.write('    <h3>Open AMR Issue</h3>\\n');
       tmp.write('    <form name=\"open-issue\" id=\"open-issue\" enctype=\"multipart/form-data\" action=\"https://www.isi.edu/cgi-bin/div3/mt/amr-editor/set-status-v1.1.cgi\" method=\"post\">\\n');
       tmp.write('      <table>\\n');
       tmp.write('        <tr><td colspan=\"2\">Open an issue regarding the AMR for snt. ' + snt_id + ' as annotated by ' + annotator + '</td></tr>\\n');
       tmp.write('        <tr><td>Type of issue:</td><td><select id=\"new-issue-type\" name=\"new-issue-type\"><option value=\"check-for-error\" selected>check for error</option><option value=\"other\">other</option></select></td></tr>\\n');
       tmp.write('        <tr><td><span title=\"Fill in this slot with the (primary) message to the annotator.&#xA;If there are several errors in an AMR, you can open multiple AMR issues.\" style=\"border-bottom:1px dotted;\">Description <nobr>of issue:</nobr><\/span></td><td><input type=\"input\" id=\"description\" name=\"description\" value=\"\" size=\"67\"></td></tr>\\n');
       tmp.write('        <tr><td><span title=\"This field is for supplementary information only.&#xA;Typically, this field is just left blank.&#xA;It will only be shown to annotator as a mouse-over.\" style=\"border-bottom:1px dotted;\">Comment:<\/span><br><span style=\"color:#999999;\">(optional)</span></td><td><textarea rows=\"3\" cols=\"46\" id=\"comment\" name=\"comment\" style=\"overflow:auto;\"></textarea></td></tr>\\n');
       tmp.write('        <tr><td colspan=\"2\"><input type=\"submit\" value=\"open AMR issue\"></td></tr>\\n');
       tmp.write('      </table>\\n');
       tmp.write('      <input type=\"hidden\" id=\"new-status\" name=\"new-status\" value=\"open\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"annotator\"  name=\"annotator\"  value=\"' + annotator + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"snt-id\"     name=\"snt-id\"     value=\"' + snt_id    + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"username\"   name=\"username\"   value=\"' + username  + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"logsid\"     name=\"logsid\"     value=\"' + logsid    + '\">\\n');
       tmp.write('    </form>\\n');
       tmp.write('  </body>\\n');
       tmp.write('</html>\\n');
...
";
  } elsif ($function_name eq "close_amr_issue") {
   $add_javascript_function_s .= "
    function close_amr_issue (annotator, snt_id, issue_id, username) {
       var logsid = get_logsid();
       if ((logsid == '') && ((s2 = document.getElementById('logsid')) != null)) {
          logsid = s2.value;
       }
       newwindow= window.open('','Close AMR issue','height=300,width=450,resizable=1,scrollbars=1,toolbar=1,statusbar=1,menubar=1');
       newwindow.focus();
       var tmp = newwindow.document;
       tmp.write('<html>\\n  <head><title>Close AMR issue</title></head>\\n');
       tmp.write('  <body style=\"background-color:#FFFFEE;\" onload=\"document.getElementById(\\'close-issue\\').submit();document.getElementById(\\'info\\').value=\\'Closed AMR issue\\';\">\\n');
       tmp.write('    <h3>Close AMR Issue</h3>\\n');
       tmp.write('    <form name=\"close-issue\" id=\"close-issue\" enctype=\"multipart/form-data\" action=\"https://www.isi.edu/cgi-bin/div3/mt/amr-editor/set-status-v1.1.cgi\" method=\"post\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"new-status\" name=\"new-status\" value=\"closed\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"annotator\"  name=\"annotator\"  value=\"' + annotator + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"snt-id\"     name=\"snt-id\"     value=\"' + snt_id    + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"issue-id\"   name=\"issue-id\"   value=\"' + issue_id  + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"username\"   name=\"username\"   value=\"' + username  + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"logsid\"     name=\"logsid\"     value=\"' + logsid    + '\">\\n');
       tmp.write('    </form>\\n');
       tmp.write('    <span name=\"info\" id=\"info\" >Closing AMR issue ...</span>\\n');
       tmp.write('  </body>\\n');
       tmp.write('</html>\\n');
       tmp.close();
       if ((s3 = document.getElementById(('close-' + issue_id))) != null) {
	  s3.innerHTML = '<span style=\"color:#007700;font-weight:bold;\">closed</span>';
       }
    }
";
  } elsif ($function_name eq "check_amr") {
   $add_javascript_function_s .= "
    function amr_check (annotator, workset, source) {
       var logsid = get_logsid();
       if ((logsid == '') && ((s2 = document.getElementById('logsid')) != null)) {
	  logsid = s2.value;
       }
       var action = 'https://www.isi.edu/cgi-bin/div3/mt/amr-editor/check-amr.cgi';
       var newwindow_title = 'check ' + annotator + ' ' + workset;
       var newwindow=window.open('',newwindow_title);
       var tmp = newwindow.document;
       tmp.write('<html>\\n  <head><title>AMR checker</title></head>\\n');
       tmp.write('  <body style=\"background-color:#FFFFEE;\" onload=\"document.getElementById(\\'amr-check\\').submit();\">\\n');
       tmp.write('Checking AMRs ...\\n');
       tmp.write('    <form name=\"amr-check\" id=\"amr-check\" enctype=\"multipart/form-data\" action=\"' + action + '\" method=\"post\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"logsid\" name=\"logsid\" value=\"' + logsid + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"username\" name=\"username\" value=\"' + annotator + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"workset\" name=\"workset\" value=\"' + workset + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"source\" name=\"source\" value=\"' + source + '\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"coverage\" name=\"coverage\" value=\"yes\">\\n');
       tmp.write('      <input type=\"hidden\" id=\"snt_id\" name=\"snt_id\" value=\"all\">\\n');
       tmp.write('    </form>\\n');
       tmp.write('  </body>\\n');
       tmp.write('</html>\\n');
       tmp.close();
...
";
  # END EXCLUDE FROM RELEASE UROMAN
  }
 }
...
sub levenshtein_distance {
   ...
   my $j;
   my @distance;
   my @s1_chars = $utf8->split_into_utf8_characters($s1, "return only chars");
   my $s1_length = $#s1_chars + 1;
   my @s2_chars = $utf8->split_into_utf8_characters($s2, "return only chars");
...
sub markup_parts_of_string_in_common_with_ref {
   ...
   my $j;
   my @distance;
   my @s_chars = $utf8->split_into_utf8_characters($s, "return only chars");
   my $s_length = $#s_chars + 1;
   my @ref_chars = $utf8->split_into_utf8_characters($ref, "return only chars");
